...
webroot = '.'
kw_file_pro = './keywords/nuc_kw.txt'	# keyword list file of nuccore
ul_file_pro = './logs/nuc_ul.txt'	# url list file of nuccore
ul_err_file_pro = './logs/nuc_ul_error.txt'	# url error log file of nuccore
lh_err_file_pro = './logs/nuc_lh_error.txt' # local html file error log file of nuccore
local_html_dir = './local_html/web/nuccore'
base_dir = webroot + '/html/nuccore/'
#base_url_pro = 'http://www.ncbi.nlm.nih.gov/protein'
base_url_pro = 'http://localhost/biss/nuccore'
...
+------------+---------------+------+-----+---------+-------+
	...
	sql = 'insert into biss_protein values (\'' + vo.getId() + '\', \'' + vo.getLocus() + '\', \'' + vo.getDef() + '\', \'' + vo.getAcc() + '\', \'' + vo.getVer() + '\', \'' + vo.getDbl() + '\', \'' + vo.getKw + '\', \'' + vo.getSrc() + '\', \'' + vo.getOrg() + '\', \'' + vo.getCmt() + '\', \'' + vo.getCtg() + '\');'
...
def query_from_db( pid):
	...
	sql = 'call ' + proc_name + 'query_nuc_by_id_proc ( \'' + pid + '\');';
...
#	patt_id = 'UniProtKB/Swiss-Prot:(.*?)</p>'	# item id
	...
	patt_dbs = 'DBSOURCE(.*?)[A-Z][A-Z][A-Z][A-Z][A-Z][A-Z]'		# dbsource
